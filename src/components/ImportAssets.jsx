...
// ── Auto-map category strings to our enum ─────────────────────────────────────
...
function autoMap(headers) {
  ...
  headers.forEach(h => {
    ...
    if (!map.name     && /name|asset|label|title|description|desc/.test(s)) map.name = h;
    else if (!map.value    && /value|amount|price|worth|val|montant/.test(s)) map.value = h;
...
// ── Parse Google Sheets URL → CSV export URL ──────────────────────────────────
...
// ── Parse raw rows + mapping → asset objects ──────────────────────────────────
function mapRows(rows, mapping, defaultCurrency, defaultCategory) {
  return rows
    .filter(r => Object.values(r).some(v => v !== '' && v != null))
    .map(r => {
      const name     = mapping.name     ? String(r[mapping.name] ?? '').trim() : '';
      const rawVal   = mapping.value    ? r[mapping.value] : '';
      ...
      const category = rawCat ? guessCategory(rawCat) : defaultCategory;
      const notes    = mapping.notes    ? String(r[mapping.notes] ?? '').trim() : '';
      return { name, value, currency, category, notes, _valid: !!name && value > 0 };
    });
}
...
function isDuplicate(row, existing) {
  return existing.some(a =>
    a.name.toLowerCase() === row.name.toLowerCase() && a.category === row.category
  );
}
...
// ── Main ──────────────────────────────────────────────────────────────────────
export default function ImportAssets({ clerkUserId, existingAssets, onImported, onClose }) {
  const { T } = useTheme();
  ...
  async function handleSheetsUrl() {
    ...
    if (!csvUrl) { setSheetsError('URL Google Sheets invalide.'); return; }
    setLoading(true);
    try {
      const r = await fetch(csvUrl);
      if (!r.ok) throw new Error('Impossible de récupérer le fichier. Vérifiez que le partage est activé.');
  ...
  const preview = step >= 2 ? mapRows(rawRows, mapping, defaultCurrency, defaultCategory) : [];
  const valid   = preview.filter(r => r._valid);
  const dupes   = preview.filter(r => r._valid && isDuplicate(r, existingAssets));
  ...
  async function doImport() {
    ...
    try {
      ...
      const r = await fetch(`${API}/api/assets/${clerkUserId}/bulk`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ assets: valid }),
      });
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 22 }}>
          <div style={{ fontFamily: T.sans, fontSize: 15, fontWeight: 600, color: T.fg0 }}>
            Import assets
          </div>
          <button onClick={onClose} style={{
            background: 'none', border: 'none', cursor: 'pointer',
            color: T.fg3, fontSize: 18, lineHeight: 1, padding: 4,
          }}>×</button>
        </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div
              ...
              style={{
                ...
                background: dragging ? `${T.greenBright}06` : T.bg2,
                transition: 'all 150ms',
              }}>
              <input ref={fileRef} type="file" accept=".csv,.xlsx,.xls" style={{ display: 'none' }}
                onChange={e => { const f = e.target.files?.[0]; if (f) handleFile(f); }} />
              <div style={{ fontSize: 28, marginBottom: 10 }}>📂</div>
              <div style={{ fontFamily: T.sans, fontSize: 13, color: T.fg1, marginBottom: 4 }}>
                Drop your file here or click to browse
              </div>
              <div style={{ fontFamily: T.sans, fontSize: 11, color: T.fg3 }}>
                CSV, Excel (.xlsx, .xls)
              </div>
            </div>
            ...
            <div>
              <div style={{ fontFamily: T.sans, fontSize: 11, color: T.fg2, marginBottom: 7 }}>
                Google Sheets URL{' '}
                <span style={{ color: T.fg3 }}>(sheet must be set to "Anyone with the link can view")</span>
              </div>
        ...
        {step === 1 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            <div style={{ fontFamily: T.sans, fontSize: 12, color: T.fg2 }}>
              {rawRows.length} rows detected. Map your columns below.
            </div>

            {/* Column mapping table */}
            <div style={{ background: T.bg2, borderRadius: 10, padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
              {['name', 'value', 'currency', 'category', 'notes'].map(field => (
                <div key={field} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                  <div style={{ fontFamily: T.sans, fontSize: 12, color: field === 'name' || field === 'value' ? T.fg0 : T.fg2, width: 80, flexShrink: 0 }}>
                    {field}{(field === 'name' || field === 'value') && <span style={{ color: '#e05555' }}> *</span>}
                  </div>
                  <select
                    value={mapping[field] ?? ''}
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontFamily: T.sans, fontSize: 10, color: T.fg2, textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 6 }}>
                  Default currency <span style={{ color: T.fg3 }}>(if no currency column)</span>
                </div>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontFamily: T.sans, fontSize: 10, color: T.fg2, textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 6 }}>
                  Default category <span style={{ color: T.fg3 }}>(if no category column)</span>
                </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: T.sans, fontSize: 11 }}>
                <thead>
                  <tr style={{ background: T.bg2, position: 'sticky', top: 0 }}>
                    {['Name', 'Category', 'Value', 'Currency', 'Status'].map(h => (
                ...
                <tbody>
                  {preview.map((row, i) => {
                    const isDupe = row._valid && isDuplicate(row, existingAssets);
                    ...
                    return (
                      <tr key={i} style={{ borderBottom: `1px solid ${T.border0}`, opacity: row._valid ? 1 : 0.4 }}>
                        ...
                        <td style={{ padding: '7px 12px', color: T.fg0, fontFamily: T.mono }}>{row.value.toLocaleString()}</td>
            ...
            {dupes.length > 0 && (
              <div style={{ fontFamily: T.sans, fontSize: 11, color: '#ffb74d', background: 'rgba(255,183,77,0.08)', border: '1px solid rgba(255,183,77,0.2)', borderRadius: 8, padding: '10px 14px' }}>
                ⚠ {dupes.length} asset{dupes.length > 1 ? 's' : ''} may already exist. They will still be imported — delete duplicates manually afterwards if needed.
              </div>
            )}
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              ...
              <button onClick={doImport} disabled={loading || valid.length === 0} style={{
                ...
                {loading ? 'Importing…' : `Import ${valid.length} asset${valid.length !== 1 ? 's' : ''}`}
              </button>
            </div>
          </div>
        )}
        ...
        {importDone != null && (
          <div style={{ textAlign: 'center', padding: '20px 0' }}>
            ...
            <div style={{ fontFamily: T.sans, fontSize: 15, color: T.fg0, marginBottom: 6 }}>
              {importDone} asset{importDone !== 1 ? 's' : ''} imported
            </div>
            <button onClick={onClose} style={{
